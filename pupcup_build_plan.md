...
_Last updated: 2026-06-02 — **milestone 14 done** (deployment artifacts): the `deploy/` directory now exists with the four production-install files. **`deploy/config.example.yaml`** is the shipped config — every field is the built-in default, commented, and a throwaway loader test confirmed it parses, validates, and equals `config.Default()` exactly (so the shipped file can't silently drift). **`deploy/pupcup.service`** is the systemd unit — `Type=notify` (the daemon already does sd_notify readiness + a 60 s watchdog), runs as the `pupcup` system user with `SupplementaryGroups=gpio i2c spi` for hardware access, `StateDirectory`/`RuntimeDirectory` for `/var/lib/pupcup` + `/run/pupcup`, `ProtectSystem=strict` + `ReadWritePaths` + `NoNewPrivileges` hardening, and `AmbientCapabilities=CAP_NET_BIND_SERVICE` so the non-root user can bind `:80`. **`deploy/bootstrap.sh`** runs once on a fresh Pi (idempotent): creates the nologin `pupcup` user + group membership, the `/opt/pupcup` · `/var/lib/pupcup{,/photos}` · `/etc/pupcup` dirs, installs the config (without clobbering an edited one) and the unit, and `enable`s the service. **`deploy/deploy.sh`** is the repeat-update path: cross-compiles `linux/arm64` pure-Go with `-trimpath -ldflags "-s -w -X main.version=$(git rev-parse --short HEAD)"`, rsyncs to the Pi, and `sudo install`s the root-owned binary + restarts. **Correction to the §10.1 snippet:** `TARGET` defaults to `scotty@pupcup.local` (the admin sudoer), not `pupcup@…` — the service user is `nologin`, so SSH logs in as the sudoer and the daemon merely *runs* as `pupcup`. Verified: `bash -n` clean on both scripts, the example-config loader test passes, and the exact deploy.sh build line produced a stripped, version-stamped arm64 ELF. Only the on-device install + the §13 UAT pass (milestone 15) remain._
_Earlier on 2026-06-02 — **milestone 10.5 done** (add-in meal tags, end-to-end): a feeding can now carry **add-in tags** (the extras mixed into a meal — shredded chicken, cheese, …) from both the web app and the physical device. **Data:** migration `0002_feed_tags.sql` adds `feed_tags` (a household catalog, Title-Cased + `NOCASE`-unique among live tags) and the `feeding_tags` join, seeds the reserved **Unspecified** sentinel (id 1, "Other / name later") first and the eight starter tags (ids 2–9, mirroring `seed.FeedTags()`); `domain.FeedTag` + `Feeding.Tags` land; the store gains `TagsForDog` (per-dog usage ranking — `COUNT(f.id)`, not the join row, so cross-dog uses don't leak), tag CRUD (`ListTags`/`CreateTag`/`GetOrCreateTag`/`RenameTag`/`ArchiveTag`), `AttachTag`/`DetachTag`/`SetFeedingTags`, and tag-loading on `GetFeeding`/`ListFeedings` (batched). **Web:** a `/tags` catalog page (create/rename/archive, PRG + methodOverride like `/dogs`; the sentinel is read-only) and inline **add-in chips** on each `/feedings` meal row — add via a per-dog ranked type-ahead datalist (creates-on-the-fly), remove via a chip ×; a feeding still carrying the sentinel shows a **"name this add-in"** affordance, and naming it attaches the real tag + clears the sentinel in one click (HTMX `POST/DELETE /feedings/{id}/tags[/{tagID}]`). **Device:** the buttons driver is upgraded to **both-edge** (press/release, `ButtonEvent.Action`) so the state machine tracks the live held-button set; Idle now uses a **deferred commit** (meal press opens an in-memory pending feeding, release commits a plain meal; rotary is ignored while a meal is held; last-wins on overlapping meals), the **add-in chord** (hold-meal-then-Blue **or** hold-Blue-then-meal) opens a new **`AddInSelect`** mode + `AddInSelectScene` (rotary scrolls the per-dog-ranked picker with a trailing "Other"; short-press commits with that tag, "Other" attaches the sentinel, an `addin_idle_seconds` walk-away commits untagged), **Blue-alone still routes to SnackMode**, and **Blue long-press in LockedSummary** (detected on the tick) enters SnackMode. Builds/vets/tests green (store tag CRUD/ranking/attach-detach + canonicalization, web catalog/chip/name-it/datalist, and device deferred-commit/both-chords/Other/timeout/last-wins/blue-long-press/rotary-ignored tests); arm64 cross-compile (incl. the both-edge linux driver + all four probes) and a **live laptop smoke** (`/tags` seeded, chip attach→display→detach, dup-name reject, rename via PATCH, datalist) pass._
_Earlier on 2026-06-02 — **milestone 13 done** (per-dog detail + eating-quality chart): a new per-dog page, **`GET /dogs/{id}`**, reached from the dog's name on the dashboard and the dogs-management list. It shows a **server-rendered SVG stacked-bar chart** of eating quality — full / partial / none meals per day — over a selectable look-back window (**7 / 30 / 90 days**, default 30, chosen via a plain `?window=` GET param so it works without JavaScript), a **summary** (meal total, per-score counts + whole-percent shares, snack count), and a **read-only history table** (meals + snacks merged newest-first; edits stay on `/feedings`, with links out to `/feedings` and the filtered `/history?dog=`). The chart helper is a new minimal package, **`internal/web/chart`** — pure functions returning `template.HTML`, no JS chart libs; bar colors come from the shared palette via `.bar-full`/`.bar-partial`/`.bar-none` CSS classes so the chart never drifts from the badges elsewhere, and an all-zero window renders a friendly empty-state. Feedings are bucketed by household-local day; the window filter reuses the store's instant bounds. Unknown / soft-deleted dogs and non-numeric ids 404. Builds/vets/tests green (new chart unit tests — empty-state, per-score segments, segment omission, well-formed SVG — plus detail-page 404, render, window-range-filter, and empty-window tests); arm64 cross-compile and a **live laptop smoke** (detail links on `/` and `/dogs`, `/dogs/{id}` 200 with chart + stats + table, window 7/30/90 + bad-window→30 fallback, 404s) pass._
...
The dashboard and dogs-management/photo surfaces landed in milestone 9 (per-dog daily feeding status; dog CRUD with name·color·photo and `/photos/{id}` serving), and the **feedings & snacks CRUD surface landed in milestone 10** — the `/feedings` page (record meal/snack with a retroactive timestamp, inline edit, delete-with-confirm) plus dashboard quick-add, all HTMX-driven (htmx vendored locally, create degrading to PRG without JS). The **illness + stress event pages landed in milestone 11** (`/illness`, `/stress` — date-range events with an "ongoing" toggle and a one-click "set end" action; stress events can be household-wide), the **unified history timeline landed in milestone 12** (`GET /history` — every meal/snack/illness/stress merged newest-first, filterable by dog/type/date-range via plain GET query params; read-only, no JS required), and the **per-dog detail page + eating-quality chart landed in milestone 13** (`GET /dogs/{id}` — a server-rendered SVG stacked-bar chart of full/partial/none meals per day over a 7/30/90-day window, a per-score summary, and a read-only meal/snack history table; the chart helper is the new `internal/web/chart` package), and the **add-in meal-tags feature landed end-to-end in milestone 10.5** (the `feed_tags`/`feeding_tags` schema + Unspecified sentinel, `domain.FeedTag`/`Feeding.Tags`, the `TagsForDog` ranking + tag-CRUD/attach-detach store methods, the `/tags` catalog and the `/feedings` add-in chips with the "name this add-in" affordance, and the device side: both-edge buttons, deferred-commit, the add-in chord both ways + `AddInSelect`, and Blue long-press in LockedSummary). The **deployment artifacts landed in milestone 14** — `deploy/{pupcup.service,bootstrap.sh,deploy.sh,config.example.yaml}` (systemd `Type=notify` unit running as the `pupcup` user with gpio/i2c/spi access + `:80` bind capability; idempotent first-boot bootstrap; cross-compile-rsync-restart deploy; a defaults-equal-`config.Default()`-verified example config). Net-new work remaining: only the on-device install + UAT pass (milestone 15). The §6.5 behavioral questions (lock trigger, meal window, Blue long-press, deferred-commit edge cases) and the photo-serving approach were **resolved on 2026-06-02** and specified in §6.5/§7/§8; the lock-trigger and meal-window refinements (grace timeout, last-meal-timed expiry, snack marker) are now **implemented** (milestone 7.5), and the deferred-commit flow, the add-in chord + `AddInSelect`, and the Blue-long-press both-edge upgrade are **implemented** (milestone 10.5). The SQLite durability stance is `synchronous=NORMAL` (no corruption on power loss; the last feeding(s) may be lost on a hard cut and re-added retroactively).
...
> ✅ **Completed** — all `internal/` packages exist and build, including `internal/web/` (the shell plus the milestone-9 dashboard + dogs-management/photo surfaces) and `internal/mdns/`; `cmd/pupcup/main.go` is wired and drives the device + web + mDNS; and `deploy/` now ships the four install artifacts (milestone 14). Per-path status is annotated inline below. **Note:** tests are co-located as `<pkg>_test.go` inside each package (no top-level `test/` dir). Web handlers live flat in the `web` package root (`web.go`/`dogs.go`/`photos.go`) rather than the originally-sketched `handlers/` subdir.
...
│   │   ├── buttons/        # 4-button driver (both-edge press/release, periph.io) — ✅ (10.5)
│   │   ├── rotary/         # KY-040 Buxton table decoder + button + long-press — ✅
│   │   ├── oled/           # SSD1306 wrapper + screen renderer (incl. AddInSelectScene) — ✅ (10.5)
│   │   ├── neopixel/       # SK6812 pure-Go SPI driver (3-bit WS encoding) — ✅
│   │   └── state/          # device state machine — ✅ (Idle/Locked/Snack/AddInSelect, deferred-commit + chord, m10.5)
│   ├── web/                # 🟡 shell ✅ (m8) + dashboard/dogs ✅ (m9) + feedings/snacks ✅ (m10) + illness/stress ✅ (m11) + history ✅ (m12) + per-dog detail/chart ✅ (m13) + add-in tags ✅ (m10.5)
...
│   │   ├── tags.go         # /tags catalog CRUD + /feedings add-in chip attach/detach handlers — ✅ (10.5)
│   │   ├── templates/      # *.html — base + dashboard/dogs/feedings/illness/stress/history/dog_detail/tags + 404; tag_area chip partial ✅ (10.5)
...
├── deploy/                 # ✅ (14) production install artifacts
│   ├── pupcup.service      # systemd Type=notify unit (pupcup user, gpio/i2c/spi, :80 cap) — ✅ (14)
│   ├── deploy.sh           # cross-compile + rsync + sudo install + restart — ✅ (14)
│   ├── bootstrap.sh        # first-time install on a fresh Pi (creates user, dirs, perms) — ✅ (14)
│   └── config.example.yaml # shipped config (== config.Default(), verified) — ✅ (14)
├── migrations/             # SQL DDL files, numbered: 0001_init.sql, 0002_feed_tags.sql — ✅ (core tables + feed_tags/feeding_tags + Unspecified sentinel + 8 starter tags)
├── go.mod
├── go.sum
├── README.md
...
> ✅ **Done** — `dogs`, `feedings`, `snacks`, `illness_events`, `stress_events`, and `device_state` are created in `0001_init.sql`; `feed_tags`/`feeding_tags`, the reserved Unspecified sentinel (id 1), and the 8 starter tags (ids 2–9) are created/seeded in `0002_feed_tags.sql` (milestone 10.5). The live migration additionally carries `dogs.deleted_at` and extra `idx_*_ts` indexes not shown below.
...
> ✅ **Done** — every type below, including `FeedTag` and `Feeding.Tags`, is implemented in [internal/domain/domain.go](internal/domain/domain.go) with `Valid()`/`Validate()` helpers (`FeedTag`/`Tags` landed in milestone 10.5).
...
> ✅ **Done (milestone 10.5)** — `feed_tags`/`feeding_tags` exist (migration 0002), and `store.TagsForDog` returns `RankedTag{ID, Name, Uses}` ordered per-dog-uses-desc then name. The ranking counts `COUNT(f.id)` (the joined feeding matched to this dog), **not** the join row, so a tag's uses on *other* dogs don't inflate the rank; the Unspecified sentinel and archived tags are excluded. Backs both the device `AddInSelect` picker and the web type-ahead datalist.
...
> ✅ **Done (milestone 10.5)** — the driver now does **both-edge** detection: a falling edge re-reads Low → `ActionPress`, a rising edge re-reads High → `ActionRelease`, with the per-pin debounce and a level-change guard rejecting bounce. `ButtonEvent` carries `Action`; the `Fake` gained `Press`/`Release`/`Tap`, the state-machine tests fire press+release pairs, and `cmd/hwprobe/buttons` prints the action. (Press-only `Fake` + hardware-verified linux driver were the milestone-7 baseline.)
...
> ✅ **Done** — driver, embedded fonts, and the `DogSelectorScene`/`LockedSummaryScene`/`SnackModeScene`/`SplashScene` renderers are implemented in [oled_linux.go](internal/device/oled/oled_linux.go) + [scenes.go](internal/device/oled/scenes.go) and hardware-verified. `AddInSelectScene`/`AddInChoice` and their renderer landed in milestone 10.5 (a header with the pending dog + score, then a scrolling choice list with the highlighted row marked by a caret + outline box — the font draws "on" pixels only, so selection is framed rather than inverted). _(The scene signatures below include the `Now time.Time` field the renderer uses.)_
...
> ✅ **Implemented (milestones 7.5 + 10.5).** `Idle`, `LockedSummary`, `SnackMode`, and now `AddInSelect` are implemented, unit-tested, and run by `cmd/pupcup/main.go` end-to-end. Covered: dog scroll, the **deferred-commit** meal flow (meal-button press opens an in-memory pending feeding for the selected dog; release commits a plain meal; rotary is ignored while a meal button is held; last-wins on overlapping meals), auto-advance, the all-dogs-fed lock, the **15-minute grace-timeout lock path** (locks a partial session; un-fed dogs left unrecorded), **last-meal-timed expiry** (`locked_until = last_meal_ts + meal_lock`), the **snack marker**, persistence/rehydration across restart, the rotary long-press override, the **add-in chord** in both orders (hold-meal-then-Blue / hold-Blue-then-meal) → `AddInSelect` (rotary picker of per-dog-ranked tags + "Other"; short-press commits with the tag, "Other" attaches the Unspecified sentinel, an `addin_idle_seconds` walk-away commits untagged; the all-fed re-check fires after the commit), **Blue-alone still → SnackMode**, and **Blue long-press in LockedSummary → SnackMode** (timestamped on the both-edge press, detected on the 1 s tick; the trailing release is consumed silently). The held-button set is tracked from the §6.1 both-edge events, so the chord-vs-snack disambiguation has no timing race.
...
> 🟡 **Mostly completed** — the loader, `PUPCUP_*` env overrides, fail-fast validation, and the accessors are implemented in [config.go](internal/config/config.go), **including** `meal_complete_grace_minutes` (`MealCompleteGrace()`), `addin_idle_seconds` (`AddInIdle()`), and `photo_max_kb`/`photo_max_px`, plus `ButtonDebounce()`/`RotaryDebounce()` helpers. The shipped `deploy/config.example.yaml` landed in milestone 14 and is verified (by a throwaway loader test) to parse, validate, and equal `config.Default()`.
...
> ✅ **Done (milestone 14)** — `deploy/` ships all four artifacts: `pupcup.service`, `bootstrap.sh`, `deploy.sh`, and `config.example.yaml`. The snippets below match the committed files (the deploy.sh `TARGET` default was corrected to the admin sudoer — see 10.1). Verified on the laptop (`bash -n` clean, example-config-equals-`Default()` loader test, exact arm64 build line → a stripped version-stamped ELF); the on-device install runs during the milestone 15 UAT.
...
`deploy/deploy.sh` (cd's to the repo root; `TARGET` defaults to the admin sudoer — **not** the `pupcup` service user, which is `nologin`):
```sh
#!/usr/bin/env bash
set -euo pipefail
...
ssh "$TARGET" 'sudo install -m 0755 -o root -g root /tmp/pupcup.new /opt/pupcup/pupcup \
  && rm -f /tmp/pupcup.new && sudo systemctl restart pupcup \
  && sleep 1 && sudo systemctl status pupcup --no-pager --lines=10'
```

### 10.2 systemd unit — ✅

`deploy/pupcup.service` (installed to `/etc/systemd/system/pupcup.service`):
```ini
[Unit]
...
`deploy/bootstrap.sh` runs once on a fresh Pi (idempotent; run as a sudoer, locally or over SSH). It also creates `/opt/pupcup` for the binary and **preserves** an already-edited `/etc/pupcup/config.yaml` rather than overwriting it:
```sh
sudo useradd --system --home /var/lib/pupcup --shell /usr/sbin/nologin pupcup || true
for grp in gpio i2c spi; do sudo usermod -aG "$grp" pupcup || true; done
...
10.5. ✅ **Add-in tags** — `feed_tags`/`feeding_tags` migration + store methods (incl. `TagsForDog`); tag catalog page (`/tags`); recent-feedings page (`/feedings`) with chip add/remove; device-side deferred-commit + `AddInSelect` state, `AddInSelectScene` rendering, and the both-edge buttons driver upgrade that the chord depends on. (Depends on milestone 7 for the device state machine and milestone 10 for the web feeding surfaces.) End of this milestone: holding a meal button and tapping Blue tags a meal from the device, and that tag is visible/editable on the web. _(Done: migration `0002_feed_tags.sql` (catalog + join + Unspecified sentinel id 1 + 8 starter tags); `domain.FeedTag`/`Feeding.Tags`; store `TagsForDog` (per-dog usage rank), `ListTags`/`CreateTag`/`GetOrCreateTag`/`RenameTag`/`ArchiveTag`, `AttachTag`/`DetachTag`/`SetFeedingTags`, batched tag-load on feeding reads. Web: `/tags` catalog (create/rename/archive, PRG+methodOverride, sentinel read-only) + inline `/feedings` add-in chips (ranked type-ahead datalist, create-on-the-fly, chip-× detach, "name this add-in" affordance that clears the sentinel) via `POST/DELETE /feedings/{id}/tags[/{tagID}]`. Device: both-edge buttons (`ButtonEvent.Action`); Idle deferred commit (press opens pending, release commits; rotary ignored while a meal is held; last-wins); the add-in chord both ways → `AddInSelect` + `AddInSelectScene` (rotary picker, short-press selects, "Other"→sentinel, `addin_idle_seconds` walk-away commits untagged); Blue-alone still → SnackMode; Blue long-press in LockedSummary → SnackMode on the tick. Builds/vets/tests green (store, web, device add-in tests); arm64 cross-compile (incl. both-edge linux driver + probes) and a live laptop smoke pass.)_
...
14. ✅ **systemd unit + deploy.sh + bootstrap.sh** — production install on the Pi. _(Done: `deploy/{pupcup.service,bootstrap.sh,deploy.sh,config.example.yaml}`. The unit is `Type=notify` (matching the daemon's sd_notify + watchdog), runs as the `pupcup` system user with `SupplementaryGroups=gpio i2c spi`, `StateDirectory`/`RuntimeDirectory`, `ProtectSystem=strict` + `ReadWritePaths` + `NoNewPrivileges` hardening, and `AmbientCapabilities=CAP_NET_BIND_SERVICE` for the `:80` bind. `bootstrap.sh` is idempotent (creates the nologin user, `/opt/pupcup` + `/var/lib/pupcup{,/photos}` + `/etc/pupcup`, installs the config without clobbering an edited one + the unit, enables the service). `deploy.sh` cross-compiles arm64 pure-Go with `-trimpath -ldflags "-s -w -X main.version=$(git rev-parse --short HEAD)"`, rsyncs, and `sudo install`s the root-owned binary + restarts; `TARGET` defaults to the admin sudoer (`scotty@pupcup.local`), not the nologin service user. Verified: `bash -n` clean, a throwaway loader test confirms `config.example.yaml` == `config.Default()`, and the deploy.sh build line produced a stripped version-stamped arm64 ELF. On-device install runs in milestone 15.)_
